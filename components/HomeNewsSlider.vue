      ...
      <div class="slider-wrapper">
        <div class="slider-viewport">
          <div
            class="slider-track"
            :style="{
              width: news.length * 451 + 'px',
              transform: `translateX(-${currentIndex * 451}px)`
            }"
          >
            <div v-for="item in news" :key="item.id" class="news-card">
              <NuxtImg :src="item.img" :alt="item.title" class="news-img" format="webp"  loading="lazy"/>
              <h3>{{ item.title }}</h3>
        ...
        <div class="slider-controls">
          ...
          <span class="slider-page">{{ currentIndex + 1 }}/{{ news.length }}</span>
          <button class="slider-btn right" @click="next">›</button>
        </div>
      </div>
    </div>
    <!-- 右侧空白占位 -->
    <div class="news-right-empty"></div>
  </section>
</template>
